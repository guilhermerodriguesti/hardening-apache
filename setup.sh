...
wget https://github.com/guilhermerodriguesti/hardening-apache/archive/master.zip

yum install unzip -y
unzip master.zip
mkdir -p $HOME/Backend
mv hardening-apache-master/app.py $HOME/Backend/
chown -Rv $USER:$USER $HOME/Backend/
mv hardening-apache-master/flaskapp.service /etc/systemd/system/
...
chown $USER:$USER /var/www/$hostname/html/
chown $USER:$USER /var/www/$hostname/
chown $USER:$USER /var/www/$hostname/log/
...
chown $USER:$USER /var/www/$hostname/log/requests.log
chown $USER:$USER /www/$hostname/log/error.log
